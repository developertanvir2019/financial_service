    ...
    axios
      ...
      .then((response) => {
        ...
        if (data) {
          Swal.fire(
            "Congratulations!!!",
            "Balance recharge request success",
            "success"
          );
          //   navigate("/");
        } else {
          Swal.fire("Balance Recharge Request Failed", response.error, "error");
        }
      })
      .catch((error) => {
        Swal.fire(
          "Balance Recharge Request Failed",
    ...
    <>
      ...
      >
        <div>
          ...
          <h3 className="text-2xl font-bold text-white mt-3">Recharge Req</h3>
        </div>
      </div>
      {/* send money modal */}
